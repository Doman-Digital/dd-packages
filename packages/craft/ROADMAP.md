...
| K | Snapshot v2: section roles (CTA band, footer CTA, pricing, testimonials, FAQ, process, features, team, contact), per-section geometry and page rhythm, screenshot measures (colourfulness, edge density, symmetry), a v1 reader, fingerprint v2 | Merged: dd-packages #50. `kind` keeps its v1 meaning and `role` is new, so no tell and no calibrated typicality moves until the null models and the estate are re-snapshotted. **Recalibration is not done**: `craft calibrate calibration/labels.json --fresh --out calibration/results/<date>` re-snapshots the AI set and the human set; the estate and the null models still need re-snapshotting. Record typicality before and after in `CHARACTER.md`. |
| L | Component-level sameness: component fingerprints, `craft estate compare --component`, per-component typicality, six rendered tells (`cta-band-stock`, `pricing-trio-popular`, `testimonial-avatar-carousel`, `faq-accordion-closer`, `stats-row`, `centred-everything`) | Merged: dd-packages #52. Rendered tells get their own surface (`rendered`), proved on fixture snapshots and on a real render. **No sibling line for components yet**: it needs 30 to 50 CTA.gallery examples snapshotted into `calibration/idiom/cta/` (run locally), and the null models rebuilt on snapshot v2 before per-component typicality says anything. |
...
| N | Specificity: `specificity(text, brief)`, `generic-hero-claim`, proof in context, `craft copy compare --competitor` | Merged: dd-packages #56. Proof in context ships as `unproven-claim`, on rendered pages only: a section is a rendered idea, and a source file does not show which claim sits next to which review. |
| Cal | Calibration tooling: `craft null import` (null models from other builders' pages), `craft null prompt`, `craft calibrate <labels.json>` (precision and recall per tell, targets 1 to 3), `calibration/labels.json`, and the blind-test protocol for target 5 in `docs/blind-test.md` | In review. **The first run is not recorded yet**: live pages (the human set fresh, wiredge.studio) and `--fresh` need a browser that can reach the web. |
