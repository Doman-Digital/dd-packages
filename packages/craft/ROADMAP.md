...
| K | Snapshot v2: section roles (CTA band, footer CTA, pricing, testimonials, FAQ, process, features, team, contact), per-section geometry and page rhythm, screenshot measures (colourfulness, edge density, symmetry), a v1 reader, fingerprint v2 | Merged: dd-packages #50. `kind` keeps its v1 meaning and `role` is new, so no tell and no calibrated typicality moves until the null models and the estate are re-snapshotted. **Recalibration is not done**: re-snapshot `calibration/estate`, the AI set and the null models, then record typicality before and after in `CHARACTER.md`. |
